...
-d %(backup_destination)s -i %(backup_id)s''' % {
    ...
    print(cron_list)
    with open('/srv/salt/crons/%s_%s.ds_cron' % (backup_server, backup_user), 'w') as f:
        f.writelines(cron_list)
    subprocess.check_call(
            "salt '%(backup_server)s' cp.get_file salt://crons/%(backup_server)s_%(backup_user)s.ds_cron /tmp/%(backup_server)s_%(backup_user)s.ds_cron"
    ...
    subprocess.check_call(
        "salt '%(backup_server)s' cron.write_cron_file_verbose %(backup_user)s /tmp/%(backup_server)s_%(backup_user)s.ds_cron"
        % {'backup_server': backup_server, 'backup_user': backup_user}, shell=True)
